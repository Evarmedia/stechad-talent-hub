...
const PMDashboard = () => {
  ...
  const activeProjects = pmDashboardData?.activeProjects ?? [];
  const statistics = pmDashboardData?.statistics;

  const stats = [
    {
      label: "Active Jobs",
      value: statistics?.activeJobsCount ?? 0,
      icon: Briefcase,
      change: `+${pmDashboardData?.recentJobs?.length ?? 0} this week`,
    },
    {
      label: "Total Applications",
      value: statistics?.totalApplicationsCount ?? 0,
      icon: Users,
      change: `+${pmDashboardData?.recentApplications?.length ?? 0} new`,
    },
    {
      label: "Projects",
      value: statistics?.activeProjectsCount ?? 0,
      icon: Clock,
      change: `${statistics?.totalProjectsCount ?? 0} completed`,
    },
    {
      label: "Success Rate",
      value: "94%",
