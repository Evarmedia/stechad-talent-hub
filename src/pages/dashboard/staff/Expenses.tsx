...
import { DollarSign, Loader2, ReceiptText } from "lucide-react";
...
const StaffExpensesPage = () => {
  ...
  const loadExpenses = useCallback(async () => {
    try {
      ...
      toast({ title: "Could not load expenses", description: getErrorMessage(error), variant: "destructive" });
    }
  }, [toast]);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Could not submit expense", description: getErrorMessage(error), variant: "destructive" });
    } finally {
