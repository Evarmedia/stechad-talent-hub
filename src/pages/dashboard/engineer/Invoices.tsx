...
const EngineerInvoicesPage = () => {
  ...
  const load = useCallback(async () => {
    try {
      const [invoiceResponse, referralResponse] = await Promise.all([
        apiService.get("staff/invoices"),
        apiService.get("referrals/dashboard"),
      ]);
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      await apiService.post("staff/invoices", {
        ...form,
  ...
  return (
    <div className='p-4 md:p-8 mx-auto space-y-6'>
      <div>
        ...
        <h1 className='text-2xl font-bold text-primary'>Engineer invoicing</h1>
      </div>
          ...
          <CardContent>
            <form onSubmit={submit} className='space-y-3'>
              ...
              <Textarea
                ...
                placeholder='Line items and activity summary'
                value={form.notes}
                onChange={(e) =>
            ...
            <div className='grid grid-cols-2 gap-3'>
              <div className='rounded-lg border p-3'>
                ...
                <p className='font-bold'>
                  ${Number(referrals.total_rewards || 0).toLocaleString()}
                </p>
              </div>
              <div className='rounded-lg border p-3'>
                <p className='text-sm text-muted-foreground'>Pending</p>
                <p className='font-bold'>
                  ${Number(referrals.pending_rewards || 0).toLocaleString()}
                </p>
              </div>
            </div>
            <p className='text-sm text-muted-foreground'>
              {referrals.completed_referrals || 0} of{" "}
              {referrals.referrals_made || 0} referrals completed.
            </p>
          </CardContent>
        </Card>
      </div>
